...
export function PbuiPresentationRef({
  ...
  const rowClasses = [
    ...
    effectiveState.dangerousTarget ? 'text-clim-danger' : '',
  ...
  return (
    <button
      ...
      data-copy-value={presentation.copyValue ?? ''}
    >
      <PbuiText tone="muted">&lt;{presentation.type}&gt;</PbuiText>{' '}
      {interactive ? (
        <PbuiClickableText as="span" tone={effectiveState.dangerousTarget ? 'danger' : 'normal'} className={labelClasses}>
          {presentation.label}
        </PbuiClickableText>
      ) : (
        <PbuiText tone={effectiveState.dangerousTarget ? 'danger' : effectiveState.muted ? 'removed' : 'bright'} className={effectiveState.selected ? 'animate-pulse' : undefined}>{presentation.label}</PbuiText>
      )}{' '}
