...
func Generate(plan ScaffoldPlan) ([]GeneratedFile, error) {
	...
	for _, component := range plan.Components {
		for _, file := range component.Files {
			content, err := renderComponentFile(component, file)
			if err != nil {
				return nil, err
			}
			files = append(files, GeneratedFile{Path: file.Path, Kind: file.Kind, Content: content})
...
func renderComponentFile(component ComponentPlan, file PlannedFile) ([]byte, error) {
	switch file.Kind {
	case "component":
		return []byte(renderComponent(component)), nil
	case "types":
		return []byte(renderTypes(component)), nil
	case "metadata":
		content, err := RenderMetadataSidecar(component)
		if err != nil {
			return nil, err
	...
	case "stories":
		return []byte(renderStories(component)), nil
	case "style":
		return []byte(renderStyles(component)), nil
	case "barrel":
		return []byte(renderBarrel(component)), nil
	case "adapter_todo":
		return []byte(renderAdapterTODO(component)), nil
	case "readme":
...
func renderTypes(component ComponentPlan) string {
	var b bytes.Buffer
	b.WriteString(generatedHeader)
...
func renderComponent(component ComponentPlan) string {
...
export default %s;
`, generatedHeader, component.ComponentName, component.ComponentName, component.ComponentName, component.ComponentName, component.ComponentName, component.TemplateID, strings.Join(component.RealizesRepresentations, ", "), strings.Join(component.RealizesActions, ", "), component.ComponentName, component.ComponentName, defaultVisualState(component), component.TemplateID, strings.Join(component.RealizesRepresentations, ","), strings.Join(component.RealizesActions, ","), component.ComponentName, tsStringArray(component.Slots), component.ComponentName, component.ComponentName)
}

func renderStories(component ComponentPlan) string {
...
`, generatedHeader, component.ComponentName, component.ComponentName, component.ComponentName, component.ComponentName, tsString(storyDescription(component)), component.ComponentName, defaultVisualState(component))
}
...
func renderBarrel(component ComponentPlan) string {
	return fmt.Sprintf("%sexport * from \"./%s\";\nexport * from \"./%s.types\";\n", generatedHeader, component.ComponentName, component.ComponentName)
}

func renderAdapterTODO(component ComponentPlan) string {
	var b bytes.Buffer
	b.WriteString(generatedHeader)
